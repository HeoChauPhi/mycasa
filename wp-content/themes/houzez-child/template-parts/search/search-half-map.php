			...
			if ($layout) {
				...
				foreach ($layout as $key=>$value) { $i++;
					...
					if(in_array($key, houzez_search_builtIn_fields())) {
						...
						echo '</div>';
						?>
						<!-- <div class="<?php //echo $common_class . ' ' . $class_flex_grow; ?>">
							<div class="form-group"><input name="property_map_address" type="text" class="houzez_search_ajax form-control" value="<?php //if (isset($_GET['property_map_address'])) {echo $_GET['property_map_address']; } ?>" placeholder="Address"></div>
						</div> -->
						<?php
					}
				}
