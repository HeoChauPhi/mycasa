...
if( ! has_post_thumbnail( $post->ID ) || get_the_post_thumbnail($post->ID) == "" ) {
	...
	if(!get_field('gallery_upload')) {
		$featured_image_url = get_field('gallery_upload')[0];
	} elseif (!get_field('gallery_url')) {
		$featured_image_url = get_field('gallery_url')[0]['picture_attachement_ids'];
	}
	
	$featured_image_url = houzez_get_image_placeholder_url('large');
}
...
<?php } elseif($top_area == 'v5') { ?>
	...
	<div class="tab-pane <?php echo esc_attr($gallery_active); ?>" id="pills-gallery" role="tabpanel" aria-labelledby="pills-gallery-tab">
		<?php get_template_part('property-details/partials/image-count'); ?>
		<?php get_template_part('property-details/partials/custom-gallery-variable'); ?>
	</div>
