...
class Cloudimage
{
    ...
    private function define_public_hooks()
    {
        ...
        // Filter for retrieve the URL for an attachment
        $this->loader->add_filter(
            'wp_get_attachment_url', $plugin_public, 'filter_cloudimage_wp_get_attachment_url',
            0, 2
        );

        // Filter to calculate the image sources to include in a ‘srcset’ attribute.
        $this->loader->add_filter(
            'wp_calculate_image_srcset', $plugin_public, 'filter_cloudimage_wp_calculate_image_srcset',
            0, 5
        );

        // Filter to scale an image to fit a particular size (such as ‘thumb’ or ‘medium’).
        if (!isset($this->cloudimage_options['cloudimage_disable_image_downsize_filter']) || !$this->cloudimage_options['cloudimage_disable_image_downsize_filter']) {
            $this->loader->add_filter(
        ...
        // Add inline CSS in head to improve the JavaScript mode visualization
        $this->loader->add_action('wp_head', $plugin_public, 'cloudimage_add_custom_styles');

        // Trigger the output buffering to filter the content
        $this->loader->add_action('init', $plugin_public, 'cloudimage_buffer_start');

        /*
         * Example usage of the custom filter filter_cloudimage_build_url
         *
         * Usage: filters the Cloudimage final url with pointer to a function
         *
         * Accept: one parameter: $url and return only one

        $this->loader->add_filter(
            'filter_cloudimage_build_url', $plugin_public, 'cloudimage_remove_v7',
            0, 1
        );
        */
        ...
        /** DEPRECATED
         *
         * Available until version: 2.7.3
         *
         * // The following filters are used to filter the content in JavaScript Plugin mode
         * foreach (array('the_content', 'the_excerpt', 'comment_text', 'widget_text_content', 'elementor/frontend/the_content') as $filter) {
         * // After parsing blocks and shortcodes
         * $this->loader->add_filter(
         * $filter, $plugin_public, 'filter_cloudimage_the_content',
         * 5, 1
         * );
         * }
         */
    ...
    private function is_development_mode()
    {
        ...
        if (isset($this->cloudimage_options['dev']) && $this->cloudimage_options['dev'] === 1) {
            ...
            $development_mode = (false === strpos($site_url, '.')) || (stripos($site_url, 'localhost:') !== false);
        }
